...
class TrainModule:
    ...
    def evaluate(self):
        ...
        for i in range(0, len(self.task['x_test']), self.args.batch_size_test):
            ...
            y_pred = self.params['model'](x_batch)
            # categorical_crossentropy 计算损失函数 ：  https://blog.csdn.net/qq_40661327/article/details/107034575
            loss = tf.keras.losses.categorical_crossentropy(y_batch, y_pred)
    ...
    def sparsify(self, weights):
        ...
        # https://blog.csdn.net/pearl8899/article/details/108632784 两个数组相乘
